...
(function ($, fluid) {
    ...
    fluid.XMLP._parseDTD = function (that, iB) {
        ...
        fluid.XMLP._setContent(that, fluid.XMLP._CONT_XML, iB - 9, iE + strClose.length);
    ...
    fluid.XMLP._parseElement = function (that, iB) {
        ...
        if (that.nameRegex.lastIndex < iDE) {
            ...
            while (that.m_iP < iDE) {
                ...
                var attrMatch = that.attrStartRegex.exec(that.m_xml), valRegex;
                if (!attrMatch) {
                ...
                if (that.m_xml.charCodeAt(that.attrStartRegex.lastIndex) === 61) { // =
                    valRegex = that.m_xml.charCodeAt(that.attrStartRegex.lastIndex + 1) === 34 ? that.attrValRegex : that.attrValIERegex; // "
