...
(function ($, fluid) {
    ...
    fluid.htmlParser.maybeEmitTextTag = function (that) {
        if (that.defstart !== -1) {
            var text = that.parser.getContent().substring(that.defstart, that.defend);
            var node = fluid.htmlParser.newNode(that);
            node.text = text;
            that.defstart = -1;
        }
    ...
    fluid.htmlParser.processComment = function (that) {
        var node = fluid.htmlParser.newNode(that);
        var parser = that.parser;
        node.comment = parser.getContent().substring(parser.m_cB, parser.m_cE);
    };
...
parseloop: // eslint-disable-line indent
        while (true) {
            ...
            case fluid.XMLP._ENTITY: // not generated by current parser
                that.defstart = -1;
                break; // not interested in reproducing these
            case fluid.XMLP._COMMENT:
                fluid.htmlParser.maybeEmitTextTag(that);
                fluid.htmlParser.processComment(that);
                break;
