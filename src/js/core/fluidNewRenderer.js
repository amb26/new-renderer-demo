...
// Of course, now we are using "real" DOM nodes throughout, we can't necessarily distinguish the situations where the outer
...
fluid.renderer.resolveRendererContainer = function (that, containerSpec) {
    ...
    if (!parentMarkup) {
        if (!that.resources.template || that.resources.template.parsed.noResource) {
